...
class VehicleModel:
    ...
    def update_params(self, stiffness_factor: float, steer_ratio: float) -> None:
        # | update vehicle model with a new stiffness factor and steer ratio
    ...
    def steady_state_sol(self, sa:float, u:float) -> np.ndarray:
        ...
        if u > 0.1:
            return self.dynamic_solution(sa, u, self)
        else:
            return self.kinematic_solution(sa, u, self)


    def calc_curvature(self, sa:float, u:float) -> float:
        """
        | Returns the curvature.
        | Mutliply it by speed (u) and it will give yaw rate.
    ...
    def curvature_factor(self, u:float) -> float:
        ...
        | Multiply it by wheel angle (not steering wheel angle) and it will give the curvature.
    ...
    def kinematic_solution(sa: float, u: float, VM: VehicleModel) -> np.ndarray:
        """
        | Calculates the steady solution at low speeds
        | At low speeds the tire slip is undefined, so a kinematic model is used.
    ...
    def calc_slip_factor(VM):
        """
        | The slip factor is a measure of how the curvature changes with speed it's positive for Oversteering vehicle,
        negative (usual case) otherwise.
        """
    ...
    def dynamic_solution(sa: float, u: float, VM: VehicleModel) -> np.ndarray:
        """
        | Calculates the steady state solution when x_dot = 0,
                    Ax + Bu = 0 ==> x = - A^{-1} B u
        ...
        A, B = self.create_dyn_state_matrices(u, VM)
        return - solve(A,B) * sa
