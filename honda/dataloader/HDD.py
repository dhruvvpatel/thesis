...
class HDD(Dataset):
    def __init__(self, root=root, phase='train', transform=None):
        ...
        train_sessions = img_folders[:25]
        valid_sessions  = img_folders[125:]
...
class Normalize(object):
    def __call__(self, sample):
        ...
        return {'image': img.type(torch.FloatTensor)/255,
                'label': label}


class BuildDataLoader(object):
    def __init__(self, dataset=HDD, scale=Rescale, crop=CropImg, totensor=ToTensor, normalize=Normalize):
